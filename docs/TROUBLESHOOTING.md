...
| Assetto Corsa lap stuck (e.g. at 4) | OCR on this block font, or wrong lap ROI | Use `reader: assetto_corsa` on `lap_number`; do not point ACC `lap_number_training` at this HUD |
| Assetto Corsa gear stuck (e.g. at 4) | OCR on this block font | Use `reader: assetto_corsa` on `gear` with `templates/gear_digits/` |
